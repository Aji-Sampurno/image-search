...
class TestEmbedder:
    ...
    def encode(self, image, grayscale=False):
        if grayscale:
            # Convert to Grayscale, then back to RGB (to keep 3 channels for ResNet)
...
print("1. Color (Current Best):")
q = embedder.encode(img_q, grayscale=False)
t = embedder.encode(img_t, grayscale=False)
score_color = np.dot(q, t)
print(f"Score: {score_color:.4f}")

print("\n2. Grayscale (Structure Only):")
q = embedder.encode(img_q, grayscale=True)
t = embedder.encode(img_t, grayscale=True)
score_gray = np.dot(q, t)
print(f"Score: {score_gray:.4f}")

if score_gray > score_color:
    print("\nSUCCESS: Grayscale is better.")
else:
    print("\nFAILURE: Color information is important.")

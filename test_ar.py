...
class GeM(nn.Module):
    def __init__(self, p=3, eps=1e-6):
        ...
        self.eps = eps
    def forward(self, x):
        # Global pooling over whatever H, W the input has
...
class TestEmbedder:
    ...
    def encode_squash(self, image):
        # Current Logic (Squash to 512x512)
    ...
    def encode_free_ar(self, image):
        # Resize shortest side to 512, keep AR
        tfm = transforms.Compose([
...
print("1. SQUASH (Fixed 512x512) - Current Method:")
q = embedder.encode_squash(img_q)
t = embedder.encode_squash(img_t)
print(f"Score: {np.dot(q, t):.4f}")

print("\n2. FREE AR (Short-side 512, variable H/W):")
q = embedder.encode_free_ar(img_q)
t = embedder.encode_free_ar(img_t)
print(f"Score: {np.dot(q, t):.4f}")

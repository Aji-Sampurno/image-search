...
print("\n2. Flip Left-Right:")
img_q_flr = ImageOps.mirror(img_q)
vec_q_flr = embedder.encode(img_q_flr)
score_flr = np.dot(vec_q_flr, vec_t)
print(f"Score: {score_flr:.4f}")

print("\n3. Flip Top-Bottom:")
img_q_ftb = ImageOps.flip(img_q)
vec_q_ftb = embedder.encode(img_q_ftb)
score_ftb = np.dot(vec_q_ftb, vec_t)
print(f"Score: {score_ftb:.4f}")

if score_flr > base_score or score_ftb > base_score:
    print("\nSUCCESS: Flipping helps!")
else:
    print("\nFAILURE: Flipping didn't help.")
